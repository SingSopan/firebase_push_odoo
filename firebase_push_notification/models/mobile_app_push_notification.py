...
class MobileAppPushNotification(models.Model):
    ...
    cred = credentials.Certificate("C:/Odoo 16/server/mod/firebase_push_notification/data/serviceAccountKey.json")
    ...
    def send_notification(self):
        ...
        data = self.link and {"link": self.link} or None
        message = messaging.MulticastMessage(
            notification=messaging.Notification(
                title= self.name or '',
                body= self.body or '',
            ),
            # data= {"link": "http://stlive.alugara.live/web#action=635&model=coupon.program&view_type=list&cids=1&menu_id=174"},
            data=data,
