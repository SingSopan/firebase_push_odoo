...
class MailThread(models.AbstractModel):
    ...
    def _notify_compute_recipients(self, message, msg_vals):
        ...
        if msg_type == 'comment':
            ...
            self._send_notification_custom(pids, message, msg_vals)
    ...
    def _send_notification_custom(self, pids, message, msg_vals):
        if not pids or len(pids)==0:
            return
        title = msg_vals['subject'] or 'Comment'
        title_pre = title
        link = False
        if msg_vals['message_type'] == 'notification':
            title_pre = title and title.split('#')[0] or False
            title_post = title and title.split('#')[1] or False
            rules = self.env['push.notification.rules'].browse([int(title_post)])
            link = rules.link and rules.link % (msg_vals['res_id']) or self._notify_get_action_link(link_type='view')
        notif = self.env['mobile.app.push.notification'].sudo().create({
            'name': title_pre,
            'body': msg_vals['body'],
            'link': link or self._notify_get_action_link(link_type='controller',controller='/web'),
            'send_notification_to': 'to_specefic',
            'partner_ids': [(6, 0, [x for x in pids])],
        })
